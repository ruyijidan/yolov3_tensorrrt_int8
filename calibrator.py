...
class ExampleEntropyCalibrator(trt.IInt8EntropyCalibrator2):
    ...
    def read_batch_file(self, idx):
        # def normalize(input00, mean, std):
        #     input00 = input00 / 255.0
        #     return (input00 - mean) / std

        img = self.all_files[idx]
        #normed_img = normalize(img, mean=(0.1307,), std=(0.3081,))
        #normed_img = normed_img.astype(np.float32)
        normed_img = img.astype(np.float32)
        #print("============normed_img===============:", normed_img)
        return normed_img
    ...
    def get_batch(self, names):
        try:
            # Get a single batch.
            cal_data = next(self.batches)
            # Copy to device, then return a list containing pointers to input device buffers.
            cuda.memcpy_htod(self.device_input, cal_data)
            #print("==========device_input============:", self.device_input )
            return [int(self.device_input)]
        except StopIteration:
            # When we're out of batches, we return either [] or None.
            # This signals to TensorRT that there is no calibration data remaining.
